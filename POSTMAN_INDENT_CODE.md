# Postman 风格缩进代码（可直接复制使用）

## 问题分析
Postman 的 JSON 缩进特点：
1. 使用 **2 个空格**作为缩进单位（约 14px，取决于字体）
2. 缩进更紧凑，视觉上更清晰
3. 每一级缩进都有明显的视觉差异
...
<template>
  <div class="json-node-wrapper">
    <div class="json-node-content" :style="{ paddingLeft: level > 0 ? `${level * 14}px` : '0' }">
      <!-- 展开/收起按钮 -->
      <span
          v-if="isExpandable"
          class="expand-icon"
...
<!-- Postman 风格 -->
:style="{ paddingLeft: level > 0 ? `${level * 14}px` : '0' }"
```

**关键变化：**
1. `marginLeft` → `paddingLeft`（paddingLeft 会让整个内容块缩进，视觉效果更明显）
2. `16px` → `14px`（更小的缩进单位，更接近 Postman 的 2 个空格）
...
### 方案2：使用 text-indent（更精确的缩进）
```vue
:style="{ textIndent: level > 0 ? `${level * 14}px` : '0' }"
```

### 方案3：组合使用（最接近 Postman）
```vue
:style="{ 
  paddingLeft: level > 0 ? `${level * 14}px` : '0',
  marginLeft: level > 0 ? `${level * 2}px` : '0'
}"
```
